...
export const Login: React.FC<LoginProps> = ({ onNavigateToSignUp }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError(null);
    try {
      await signInWithGoogle('fan'); // Defaults to fan. If profile already exists in DB, it will use that role
    } catch (e: any) {
      console.error(e);
      ...
      <main className="relative z-10 w-full max-w-[480px] animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="bg-surface-container login-card-blur rounded-xl p-lg shadow-2xl border border-outline-variant/20">
          <form onSubmit={handleSubmit} className="space-y-md">
            ...
            >
              ...
              <span className="font-label-caps text-label-caps tracking-wider uppercase font-bold">Sign In with Google</span>
            </button>
          </form>
        </div>
        ...
        <div className="mt-xl flex flex-col items-center opacity-40">
          <div className="flex items-center gap-xs font-data-mono text-[10px] uppercase tracking-widest text-outline">
            ...
            System Live | Node: US-SOUTH
          </div>
          <div className="mt-xs text-[10px] text-outline">© 2026 FIFA Operations & Logistics</div>
        </div>
      </main>
    </div>
  );
